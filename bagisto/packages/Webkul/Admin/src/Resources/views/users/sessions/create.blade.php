            ...
            <div class="form-container" style="text-align: left">
                ...
                <form method="POST" action="{{ route('admin.session.store') }}" @submit.prevent="onSubmit">
                    ...
                    <div class="control-group" :class="[errors.has('email') ? 'has-error' : '']">
                        ...
                        <input type="text" v-validate="'required|email'" class="control" id="email" name="email" value="admin@example.com" data-vv-as="&quot;{{ __('admin::app.users.sessions.email') }}&quot;"/>
                        <span class="control-error" v-if="errors.has('email')">@{{ errors.first('email') }}</span>
                    </div>

                    <div class="control-group" :class="[errors.has('password') ? 'has-error' : '']">
                        <label for="password">{{ __('admin::app.users.sessions.password') }}</label>
                        <input type="password" v-validate="'required|min:6'" class="control" id="password" name="password" value="admin123" data-vv-as="&quot;{{ __('admin::app.users.sessions.password') }}&quot;" value=""/>
